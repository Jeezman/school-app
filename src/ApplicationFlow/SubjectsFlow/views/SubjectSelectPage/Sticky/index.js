...
const getPositionStyle = el => {
  ...
  for (var key in initialState.style) {
    var value = parseInt(style.getPropertyValue(key), 10);
...
class ReactStickyState extends Component {
  ...
  getBounds(noCache) {
    ...
    if (noCache !== true && this.state.bounds.height !== null) {
      if (
        ...
        clientRect.height === this.state.bounds.height
      ) {
        return {
    ...
    if (!Can.sticky) {
      ...
      if (elem) {
        offsetY = parseInt(window.getComputedStyle(elem)["margin-bottom"], 10);
        offsetY = offsetY || 0;
        ...
        offsetY = parseInt(window.getComputedStyle(elem)["padding-top"], 10);
        offsetY = offsetY || 0;
        rect = getAbsolutBoundingRect(elem);
        if (this.hasOwnScrollTarget) {
          rect = addBounds(rect, getAbsolutBoundingRect(this.scrollTarget));
  ...
  getScrollClasses(obj) {
    if (this.options.scrollClass.active) {
      ...
      var direction =
        this.scroll.y <= 0 ||
        this.scroll.y + this.scroll.clientHeight >= this.scroll.scrollHeight
          ? 0
    ...
    if (this.props.scrollClass.up || this.props.scrollClass.down) {
      var direction =
        ...
        this.scroll.y + this.scroll.clientHeight >= this.scroll.scrollHeight
          ...
          : this.scroll.directionY;
      var scrollClass =
        direction < 0 ? this.props.scrollClass.up : this.props.scrollClass.down;
  ...
  onScrollDirection(e) {
    if (
      this.state.sticky ||
      (e && e.type === ScrollFeatures.events.SCROLL_STOP)
    ) {
      this.setState({
  ...
  getStickyState() {
    ...
    if (top !== null) {
      var offsetBottom =
        this.state.restrict.bottom - this.state.bounds.height - top;
      top = this.state.bounds.top - top;

      if (
        this.state.sticky === false &&
        ((scrollY >= top && scrollY <= offsetBottom) ||
          (top <= 0 && scrollY < top))
      ) {
        sticky = true;
        absolute = false;
        ...
        this.state.sticky &&
        ((top > 0 && scrollY < top) || scrollY > offsetBottom)
      ) {
        sticky = false;
      ...
      var offsetTop =
        this.state.restrict.top + this.state.bounds.height - bottom;
      ...
      if (
        ...
        this.state.sticky &&
        (scrollY > bottom || scrollY < offsetTop)
      ) {
        sticky = false;
  ...
  updateStickyState(silent) {
    ...
    if (
      values.sticky !== this.state.sticky ||
      values.absolute !== this.state.absolute
    ) {
      this._shouldComponentUpdate = silent !== true;
  ...
  render() {
    ...
    const className = classNames(
      {
        [stickyClass]: !this.state.disabled,
        [disabledClass]: this.state.disabled
      },
      {
    ...
    if (!Can.sticky) {
      if (this.state.absolute) {
        style = {
          marginTop:
            this.state.style.top !== null
              ? this.state.restrict.height -
                (this.state.bounds.height + this.state.style.top) +
                (this.state.restrict.top - this.state.bounds.top) +
                "px"
              : "",
          marginBottom:
            this.state.style.bottom !== null
              ? this.state.restrict.height -
                (this.state.bounds.height + this.state.style.bottom) +
                (this.state.restrict.bottom - this.state.bounds.bottom) +
                "px"
              : ""
        };
    ...
    if (element) {
      ...
      element = (
        <Comp ref={refName} style={style} className={className} {...props}>
          {this.props.children}
        </Comp>
      );
    }
    ...
    const height =
      ...
      this.state.bounds.height ===
        null /*|| (!this.state.sticky && !this.state.absolute)*/
        ? "auto"
        : this.state.bounds.height + "px";
    const marginTop =
      height === "auto"
        ? ""
        : this.state.style["margin-top"]
          ? this.state.style["margin-top"] + "px"
          : "";
    const marginBottom =
      height === "auto"
        ? ""
        : this.state.style["margin-bottom"]
          ? this.state.style["margin-bottom"] + "px"
          : "";
    ...
    return (
      <div ref="wrapper" className={wrapperClass} style={style}>
        {element}
      </div>
    );
  }
